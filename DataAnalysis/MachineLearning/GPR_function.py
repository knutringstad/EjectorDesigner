...
def GPR_function(df,features, output, sample_size, seed, displayBool,optimizeBool,trainBool):
    ...
    df = df.drop(df.loc[abs(df["mfr_err"])>0.01].index)
    ...
    if displayBool:
        ...
        if stdplot==True:
            ...
            plt.contourf(unscaled[:,var1],unscaled[:,var2],std_grid[:,:],levels=levels)
    ...
    if optimizeBool ==True:
        ...
        if optimizeplot:
            ...
            if diagram =="ph":
                ...
                plt.xlim([2.45e+2,3.5e+2])
                plt.xticks(np.arange(2.45e+2,3.5e+2, step=25))
        ...
        for i in range(Nstarts):
            ...
            # plt.scatter(   np.divide(x_list_unscaled[:,3],10**3),np.divide(x_list_unscaled[:,0],10**5), s=1,    color=cm.plasma(np.arange(len(x_list_unscaled[:,3])))    )
            ...
            # print(cm.plasma(np.divide(np.arange(len(x_list_unscaled[0:-1:NscatterpointsSteps,3])),len(x_list_unscaled[0:-1:NscatterpointsSteps,3]))  ))
