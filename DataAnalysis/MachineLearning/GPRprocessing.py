...
df_ejector["mfr_err"] =abs( df_ejector["mfr_err"] )
...
gp = GaussianProcessRegressor(kernel=kernel,alpha=std_estimate**2,n_restarts_optimizer=2).fit(x_train, y_train)  #run fitting
print(gp.kernel_) 
...
A=sc.inverse_transform(np.array([0.7,0,0,-1.3,0]).T)
print(A)
